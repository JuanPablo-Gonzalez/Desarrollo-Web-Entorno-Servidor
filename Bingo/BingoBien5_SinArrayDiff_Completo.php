...
/*Quise hacerlo sin array_diff  porque tras darle unas vueltas 
al código de la primera forma pensé que no era necesario.
Es muy parecido al otro pero haciendo el tachado primeramente 
y luego comprobando el ganador o ganadores.*/
	...
	for($i=1;$i<4;$i++) {
		...
		while($contNumeros<15) {
			$numAleatorio= random_int(1,60);
			 //si no está ya en el cartón, entonces se mete.
...
while(!$ganador1&&!$ganador2&&!$ganador3&&!$ganador4) {
	...
	/*AQUÍ LO QUE HAREMOS SERÁ COMPROBAR EL GANADOR, DE TAL FORMA QUE SI EL CARTÓN DEL 
	JUGADOR ES IGUAL AL ARRAY DE TACHADOS, SIGNIFICARÁ QUE ESE JUGADOR HA GANADO CON ESE CARTÓN.*/
	$cartonTachado= array("--","--","--","--","--","--","--","--","--","--","--","--","--","--","--");
	$cartonGanadorJ1;
	$cartonGanadorJ2;
	$cartonGanadorJ3;
