...
while(!$ganador1&&!$ganador2&&!$ganador3&&!$ganador4) {
	...
	AL CARTÓN DE TACHADOS, SIGNIFICARÁ QUE ESE JUGADOR HA GANADO CON ESE CARTÓN.*/
	$cartonTachado= array("--","--","--","--","--","--","--","--","--","--","--","--","--","--","--");
	$cartonGanadorJ1;
	$cartonGanadorJ2;
	$cartonGanadorJ3;
...
/*Esta es otra forma para mostrar las bolas a modo de tabla,
pero ocupaba mucho ancho de la pantalla y no quedaba bien visualmente.*/
/*
echo "<table border=1>";
echo "<tr>";
for($i=0;$i<count($numerosSacados);$i++) {
...
/* Quise hacerlo a modo de tabla, pero partiendo cada línea cada 10 bolas.
Lo conseguí haciéndolo así pero era muy rebuscado.
...
echo "</tr>";
echo "<tr>";
while($i>=40&&$i<=count($numerosSacados)-1) {
	$imagenBola= $numerosSacados[$i].".PNG";
	echo "<td> <img src='ImagenesBolas/".$imagenBola."' width='30px'> </td>"; 
	$i++;
}
